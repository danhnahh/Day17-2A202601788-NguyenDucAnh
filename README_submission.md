...
**Kết quả:** practice 11/11 (100%); baseline no-memory 2/11 (18.2%); golden 20/20.
...
**Trade-off Zep vs Redis+Qdrant.** Zep tự trích entity/fact kèm `valid_at`/`invalid_at`; một call `thread.get_user_context` trả context đã xếp hạng relevance. Cái phải trả: ingest bất đồng bộ nên phải poll, độ trễ trung bình 1625ms so với ~0ms của store local, và không kiểm soát được ranking. Redis+Qdrant cho toàn quyền và độ trễ gần 0, nhưng phải tự viết extraction, conflict resolution, recency, provenance — đúng bốn thứ tốn thời gian nhất.
...
2. **Nhiều token nhất:** bốn case long-term chiếm trọn top (726–1455 token), dẫn đầu E02, do Context Block nối thêm `graph.search(scope="edges", limit=20)`.
3. **E07** cần long-term + semantic; evidence bắt buộc `Python` (user graph) và `Idempotency-Key` (KB chung).
4. **Token reduction:** memory 19.1% vs baseline 81.8%, nhưng hit rate 100% vs 18.2%. Baseline "tiết kiệm" vì không retrieve gì. Reduction chỉ có nghĩa khi đọc kèm hit rate.
